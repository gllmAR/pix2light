...
void ofApp::update(){
    ...
    //create send buffer by ofFbo
    {
        ...
        ofSetColor(0, 0, 0);
        ofDrawLine(ofGetFrameNum()%44/2, 0, ofGetFrameNum()%44/2, 1);
        fbo.end();
    ...
    artnet.sendDmx("192.168.101.95", 0, 0, testImage.getPixels().getData(), 512);
//    artnet.sendDmx("192.168.101.178", 0, 1, testImage.getPixels().getData(), 512);
//    artnet.sendDmx("192.168.101.178", 0, 2, testImage.getPixels().getData(), 512);
//    artnet.sendDmx("192.168.101.178", 0, 3, testImage.getPixels().getData(), 512);
